...
const TOOLS = [
  {
    ...
    input_schema: { type: 'object', properties: { query: { type: 'string', description: 'palabra clave: nombre del proyecto o cliente' } }, required: ['query'] },
  },
  {
  ...
  },
  {
    ...
    input_schema: { type: 'object', properties: { query: { type: 'string', description: 'palabra clave: ej. una ruta, una ciudad, un tipo de referencia' } }, required: ['query'] },
  },
  {
    ...
    input_schema: { type: 'object', properties: { query: { type: 'string', description: 'palabra clave: ej. una ruta, un proveedor, un tipo de gasto' } }, required: ['query'] },
  },
];
...
const SYSTEM = `Sos el asistente interno de Matilda Hub, el sistema de gestión de Matilda Event Designers (agencia de eventos en Ecuador).
...
serve(async (req) => {
  ...
  try {
    if (!ANTHROPIC_API_KEY) throw new Error('Falta configurar ANTHROPIC_API_KEY en Supabase → Edge Functions → Secrets');
    ...
    for (let i = 0; i < 6; i++) {
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 1500, system: SYSTEM, messages, tools: TOOLS }),
      });
      const json = await resp.json();
      if (json.error) throw new Error(json.error.message || 'Error llamando a Claude');
      ...
      for (const tu of toolUses) {
        let resultado;
        try { resultado = await ejecutarTool(tu.name, tu.input); }
