...
serve(async (req) => {
  ...
  try {
    ...
    const { solicitud, solicitanteEmail } = JSON.parse(text);
    const total = (solicitud.items||[]).reduce((a: number,it: any)=>a+Number(it.valor_solicitado||0),0);
    const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:24px;background:#f8fafc;border-left:4px solid #2e8b4e;">
        <h2 style="color:#2e8b4e;margin:0 0 12px;">✅ Tu solicitud fue aprobada</h2>
        <p style="color:#333;"><strong>${solicitud.presupuesto_nombre || 'Sin presupuesto'}</strong></p>
        <p style="color:#666;">Monto aprobado: <strong>$${total.toFixed(2)}</strong></p>
        <a href="https://mapa-zeta.vercel.app" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#2e8b4e;color:#fff;text-decoration:none;border-radius:6px;">Ver en Matilda Hub</a>
      </div>
    ...
    const to = [solicitanteEmail].filter(Boolean);
    if (to.length > 0) await sendEmail(to, `✅ Solicitud aprobada: ${solicitud.presupuesto_nombre || ''}`, html);
