...
serve(async (req) => {
  ...
  try {
    ...
    const { propuesta } = JSON.parse(text);
    const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:24px;background:#f8fafc;border-left:4px solid #7c3aed;">
        <h2 style="color:#7c3aed;margin:0 0 12px;">📋 Nueva Propuesta</h2>
        <p style="color:#333;"><strong>${propuesta.nombre || 'Sin nombre'}</strong></p>
        <p style="color:#666;">Cliente: ${propuesta.cliente || '—'}</p>
        <a href="https://mapa-zeta.vercel.app" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;text-decoration:none;border-radius:6px;">Ver en Matilda Hub</a>
      </div>
    ...
    await sendEmail(DESTINOS, `📋 Nueva propuesta: ${propuesta.nombre || ''}`, html);
